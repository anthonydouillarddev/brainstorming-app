...
export default function FlowsExportBlock({
  ...
  const content = useMemo(() => exportFlowsMd(state, project), [state, project]);

  async function handleCopy() {
    try {
      await navigator.clipboard.writeText(content);
      setCopied(true);
      toast.success("Parcours copié dans le presse-papier");
  ...
  function handleDownload() {
    ...
    toast.success("flows.md téléchargé");
  }
      ...
      {!collapsed && (
        <>
          <div className="bg-card/40 border border-border rounded-lg p-3 text-xs text-muted leading-relaxed">
            ...
            génère les copies d&apos;onboarding, les events analytics à tracker, ou les écrans
            d&apos;empty state.
          </div>
