...
import { exportA11yMd } from "../exports/markdown";
import { exportA11yJson } from "../exports/json";
import { exportA11yClaudeBrief } from "../exports/claude-brief";
...
const FORMATS: {
...
}[] = [
  {
    ...
    hint: "Rapport d'audit · WCAG + flows + ARIA",
  },
  {
    key: "claude",
    label: "Brief Claude / ChatGPT",
    emoji: "🤖",
    filename: "brief-a11y.md",
    hint: "Prompt pour code / remediation / tests",
  },
  {
    key: "json",
    label: "JSON snapshot",
...
export default function A11yExportBlock({
  state,
  project,
}: {
  state: A11yState;
  ...
  const content = useMemo(() => {
    switch (format) {
      case "markdown":
        return exportA11yMd(state, project);
      case "claude":
        return exportA11yClaudeBrief(state, project);
      case "json":
        return exportA11yJson(state, project);
    }
      ...
      {!collapsed && (
        <>
          <div className="bg-card/40 border border-border rounded-lg p-3 text-xs text-muted leading-relaxed">
            <strong className="text-foreground">3 formats</strong> — Markdown pour rapport
            d&apos;audit, Brief Claude pour génération de code / plan de remediation, JSON pour
            back-up.
          </div>
