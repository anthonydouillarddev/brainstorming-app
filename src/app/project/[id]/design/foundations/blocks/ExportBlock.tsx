...
export default function FoundationsExportBlock({
  ...
  const content = useMemo(() => exportFoundationsMd(state, project), [state, project]);

  async function handleCopy() {
    try {
      await navigator.clipboard.writeText(content);
      setCopied(true);
      toast.success("Fondations copiées dans le presse-papier");
  ...
  function handleDownload() {
    ...
    a.href = url;
    a.download = "foundations.md";
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
    toast.success("foundations.md téléchargé");
  }
  ...
  return (
    <div className="space-y-4 border-t border-border pt-6">
      ...
      {collapsed && (
        ...
        >
          ▼ Voir l&apos;export markdown (prêt à coller dans Claude / Notion)
        </button>
      )}
      ...
      {!collapsed && (
        <>
          <div className="bg-card/40 border border-border rounded-lg p-3 text-xs text-muted leading-relaxed">
            <strong className="text-foreground">À quoi ça sert ?</strong> Colle ce document dans
            Claude/ChatGPT pour qu&apos;il génère des user flows, wireframes ou copy alignés sur
            ces fondations. Les chapitres suivants (2 à 12) pourront le réinjecter aussi.
          </div>
