...
import { exportMicrocopyMd } from "../exports/markdown";
import { exportMicrocopyJson } from "../exports/json";
import { exportMicrocopyClaudeBrief } from "../exports/claude-brief";
...
const FORMATS: {
  ...
  {
    key: "markdown",
    label: "Microcopy Markdown",
    emoji: "📄",
    filename: "microcopy.md",
    hint: "Doc lisible · copy deck",
  },
  {
    key: "claude",
    label: "Brief Claude / ChatGPT",
    emoji: "🤖",
    filename: "brief-microcopy.md",
    hint: "Prompt pour générer des variantes cohérentes",
  },
  {
    key: "json",
    label: "JSON snapshot",
    emoji: "📦",
    filename: "microcopy.json",
    hint: "Schéma Mindeck 1.0 · back-up & import",
  },
];

export default function MicrocopyExportBlock({
  state,
  project,
}: {
  ...
  const content = useMemo(() => {
    switch (format) {
      case "markdown":
        return exportMicrocopyMd(state, project);
      case "claude":
        return exportMicrocopyClaudeBrief(state, project);
      case "json":
        return exportMicrocopyJson(state, project);
    }
      ...
      {!collapsed && (
        <>
          <div className="bg-card/40 border border-border rounded-lg p-3 text-xs text-muted leading-relaxed">
            <strong className="text-foreground">3 formats</strong> — Markdown pour copy deck,
            Brief Claude pour produire des variantes cohérentes, JSON pour back-up.
          </div>
