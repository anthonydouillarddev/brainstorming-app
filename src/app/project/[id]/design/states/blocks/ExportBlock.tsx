...
import { exportStatesMd } from "../exports/markdown";
import { exportStatesJson } from "../exports/json";
import { exportStatesClaudeBrief } from "../exports/claude-brief";
...
const FORMATS: {
  ...
  {
    key: "markdown",
    label: "États Markdown",
    emoji: "📄",
    filename: "states.md",
    hint: "Doc lisible · brief designer / dev",
  },
  {
    key: "claude",
    label: "Brief Claude / ChatGPT",
    emoji: "🤖",
    filename: "brief-states.md",
    hint: "Prompt pour générer React/CSS des états",
  },
  {
    key: "json",
    label: "JSON snapshot",
...
export default function StatesExportBlock({
  state,
  project,
}: {
  state: StatesState;
  ...
  const content = useMemo(() => {
    switch (format) {
      case "markdown":
        return exportStatesMd(state, project);
      case "claude":
        return exportStatesClaudeBrief(state, project);
      case "json":
        return exportStatesJson(state, project);
    }
      ...
      {!collapsed && (
        <>
          <div className="bg-card/40 border border-border rounded-lg p-3 text-xs text-muted leading-relaxed">
            <strong className="text-foreground">3 formats</strong> — Markdown pour doc designer,
            Brief Claude pour générer React/CSS, JSON pour back-up.
          </div>
